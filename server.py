...
# File where the webhook server stores events
...
async def analyze_file_changes(
    ...
    """
    try:
        # Try to get a working directory from roots first
...
async def suggest_template(changes_summary: str, change_type: str) -> str:
    ...
    selected_template = next(
        ...
        templates[0]  # Default to the first template if no match
    )
...
async def create_deployment_summary():
    ...
    return """Create a deployment summary to communicate the status of the deployment to the team:

    1. Check the workflow status using get_workflow_status()
    2. Look specifically for the deployment workflows
    3. Identify the deployment outcome (success/failure) and any relevant details

    Format as a concise message that can be shared with the team using Slack:

    🚀 **Deployment Update**
    - **Workflow Name**: [Name of the deployment workflow]
    - **Status**: [✅Success / ❌Failed / ⏳In Progress]
    - **Environment**: [Dev / Staging / Production]
    - **Version**: [Version number or commit hash]
    - **Started**: [If available Date and time of deployment start]
    - **Finished**: [If Available Date and time of deployment completion]
    - **Key Changes**: [Brief summary of the changes deployed if available]
    - **Issues**: [Any issues or blockers that need attention]
    - **Next Steps**: [Any follow-up actions required if failed]

    Keep it brief and informative to help the team stay updated on the deployment progress."""
